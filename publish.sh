...
cat << EOF
# Run this script from the directory of the module (e.g. redshift-udfs-textanalytics) that you wish to publish.
...
if [ "$#" -lt 2 ]; then
    ...
    echo "\n2. The connector module to publish (e.g. ./redshift-udfs-textanalytics) \n"
    echo "\n3. The AWS REGION to target (e.g. us-east-1 or us-east-2) \n"
    echo "\n\n USAGE from the module's directory: ../tools/publish.sh my_s3_bucket redshift-udfs-textanalytics us-east-1\n"
    exit;
fi
